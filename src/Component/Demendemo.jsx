  ...
  return (
    <div className={`absolute bg-black/[0.4]  h-[100vh] top-0 w-full mx-auto ${showDomendForm?'block':'hidden'}`}>
      <form className='relative bg-third sm:bg-scond md:w-[65%] w-full h-screen mx-auto p-[10px] sm:px-[60px] overflow-y-auto overflow-x-hidden'>
        ...
        <div className='relative z-10 text-h5 text-center'>{t('form1.title')}</div>
        ...
        <div className='flex items-center mt-2'>
           ...
           <div className='text-n-small text-gray-darck  ml-4'>{t('form1.chkbox2')}<Link className='underline'>{t('form1.policy')}</Link></div>
        </div>
        <div className='text-center'>
          <button className='hover:bg-primary transition-all mt-4 w-full sm:w-[50%] px-[26px] py-[16px] bg-black text-white rounded-full'>{t('form1.title')}</button>
        </div>
